...
def create_appointment(client, token):
...
def test_payment_approved(client):
    ...
    appointment_id = create_appointment(client, token)
    ...
    assert response.json()["transaction_id"] == "TXN-1234"

def test_payment_rejected(client):
    token = get_token(client)
    appointment_id = create_appointment(client, token)
...
def test_payment_already_paid(client):
    token = get_token(client)
    appointment_id = create_appointment(client, token)
...
def test_payment_not_owner(client):
    token1 = get_token(client, "client1@test.com")
    token2 = get_token(client, "client2@test.com")
    appointment_id = create_appointment(client, token1)
